...
export default function RoomsPage() {
    ...
    const handleSaveRoom = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (response.ok) {
                // Fetch immediately to update UI, then rely on polling
        ...
        <div className="max-w-7xl mx-auto py-10 px-4 sm:px-6 lg:px-8 min-h-screen">
            <div className="flex justify-between items-center mb-10">
                ...
                >
                    ...
                    {isAdmin ? 'Create Room' : 'Request Class'}
                </button>
            </div>
            ...
            {loading ? (
                ...
                <div className="text-center py-20 bg-card/30 rounded-3xl border border-dashed border-border">
                    ...
                    <h3 className="text-xl font-semibold text-muted-foreground mb-2">
                        {isAdmin ? 'No rooms created yet' : 'No classes assigned'}
                    </h3>
                    <p className="text-muted-foreground mb-6">
                        {isAdmin ? 'Create your first room to get started.' : 'Request a class to get started.'}
                    </p>
                    <button
                        onClick={openCreateModal}
                        className="text-blue-500 font-medium hover:underline"
                    >
                        {isAdmin ? 'Create a Room' : 'Request a Class'}
                    </button>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {rooms.map((room) => {
                        ...
                        return (
                            <div key={room._id} className="group relative h-full">
                                {/* Card Wrapper - Conditional Click */}
                                {isPending ? (
                                    <div className="bg-card/40 backdrop-blur-xl border border-white/10 p-6 rounded-3xl shadow-sm transition-all h-full flex flex-col justify-between relative overflow-hidden opacity-75 cursor-not-allowed">
                                        {/* Content (duplicated structure for consistency) */}
                                        ...
                                        <div>
                                            <div className="flex items-center gap-3 mb-4">
                                                <div className="p-3 bg-yellow-500/10 rounded-2xl text-yellow-500">
                                                    <Loader2 className="w-6 h-6 animate-spin" /> {/* Spinner for pending */}
                                                </div>
                                                <div>
                                                    <div className="flex items-center gap-2">
                                                        <p className="text-xs text-muted-foreground font-medium uppercase tracking-wider">Class</p>
                                                        <span className="bg-yellow-500/20 text-yellow-500 text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wide">Pending Approval</span>
                                                    </div>
                                        ...
                                        <div className="flex items-center justify-between mt-4 pt-4 border-t border-white/5">
                                            <span className="text-xs text-muted-foreground">
                                                Requested {new Date(room.createdAt).toLocaleDateString()}
                                            </span>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-card w-full max-w-md p-8 rounded-3xl shadow-2xl border border-white/10 m-4 animate-in zoom-in-95 duration-200">
                        <h2 className="text-2xl font-bold mb-6">
                            {editingRoom ? 'Edit Room' : (isAdmin ? 'Create New Room' : 'Request New Class')}
                        </h2>
                        <form onSubmit={handleSaveRoom} className="space-y-4">
                            ...
                            <div className="flex gap-3 mt-8">
                                ...
                                >
                                    ...
                                    {editingRoom ? 'Save Changes' : (isAdmin ? 'Create' : 'Send Request')}
                                </button>
                            </div>
                        </form>
                    </div>
